...
function duplicate_post_make_duplicate_link_row($actions, $post) {
	if ((duplicate_post_is_current_user_allowed_to_copy()) && (get_post_type($post->ID) == 'portal_projects')) {
...
function duplicate_post_add_duplicate_post_button() {
	...
	if ( isset( $_GET['post'] ) && duplicate_post_is_current_user_allowed_to_copy()) {
		...
		if(get_post_type($post->ID) == 'portal_projects') { ?>
<div id="duplicate-action">
	<a class="submitduplicate duplication"
		href="<?php echo duplicate_post_get_clone_post_link( $_GET['post'] ) ?>&post_status=draft"><?php _e('Copy to a new draft', DUPLICATE_POST_I18N_DOMAIN); ?>
	</a>
</div>
		<?php }
	}

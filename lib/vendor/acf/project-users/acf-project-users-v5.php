	...
	*/
	...
	function __construct() {
		...
		*/
		...
		$this->label = __('Project Users', 'acf-num_slider');
		...
		*/
		?>
				...
				<div>
					...
					$post_id = ( get_query_var('portal_manage_page') == 'edit' && get_query_var('portal_manage_option') ? get_query_var('portal_manage_option') : $post->ID );
					...
					<select name="<?php echo esc_attr($field['name']); ?>" class="portal-project-users <?php echo esc_attr($field['class']); ?>">

						<?php if(($field['value']) && ($field['value'] != 'unassigned')): ?>
							<option value="unassigned"></option>
								<?php $task_user = get_user_by('id',$field['value']); ?>
							<option value="<?php echo esc_attr($field['value']); ?>" selected><?php echo esc_html($task_user->user_nicename); ?></option>
						<?php else: ?>
							<option value="unassigned"></option>
						<?php endif; ?>

						<?php
						foreach( $users as $user ): if( $user == $field['value'] ) continue;

							$user_obj = get_userdata($user); ?>

							<option value="<?php echo esc_attr($user); ?>"><?php echo esc_html($user_obj->user_nicename); ?></option>

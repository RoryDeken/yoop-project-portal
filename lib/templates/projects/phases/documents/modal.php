<div class="document-update-dialog portal-hide portal-modal" id="portal-du-doc-<?php esc_attr_e($document['index']); ?>">
    <form method="post" action="<?php echo esc_url(get_permalink($post_id)); ?>" class="document-update-form">
        ...
        <?php if( is_user_logged_in() && get_post_type() == 'portal_projects' ) {
            apply_filters( 'portal_document_update_form_fields', portal_the_document_form_fields( $post_id, $document['index'], $document['title'], get_current_user_id() ) );
        } ?>

        <div class="portal-document-form">

            <h4><?php esc_html_e( 'Update Status', 'portal_projects' ); ?><strong><?php echo esc_html($document['title']); ?></strong></h4>
            ...
            <p><label for="portal-doc-status-field"><?php _e('Status','portal_projects'); ?></label>
                <div class="portal-select-wrapper">
                    <select class="portal-doc-status-field" id="portal-pro-<?php esc_attr_e($post_id); ?>-doc-<?php echo $document['index']; ?>">
                        <?php
            ...
            <?php if( portal_get_project_users() ) { ?>
                ...
                <p class="all-line"><label for="portal-du-doc-<?php esc_attr_e($i); ?>-all">
                    <label for="portal-du-doc-<?php esc_attr_e($document['index']); ?>-all">
                        <input type="checkbox" class="all-checkbox" name="portal-notify-all" id="portal-du-doc-<?php esc_attr_e($document['index']); ?>-all" value="all"> <?php esc_html_e( 'All Users', 'portal_projects' ); ?>
                    </label>
                    <label for="portal-du-doc-<?php esc_attr_e($document['index']); ?>-specific">
                        <input type="checkbox" class="specific-checkbox" name="portal-notify-specific" value="specific" id="portal-du-doc-<?php esc_attr_e($document['index']); ?>-specific"> <?php esc_html_e( 'Specific Users', 'portal_projects' ); ?>
                    </label>
                </p>
                ...
                <ul class="portal-notify-list">
                    ...
                    foreach( $users as $user ):
                        ...
                        <li class="portal-notify-user">
                            <label for="portal-du-doc-<?php echo esc_attr( $document['index'] . '-' . $user['ID'] ); ?>">
                                <input id="portal-du-doc-<?php echo esc_attr( $document['index'] . '-' . $user['ID'] ); ?>" type="checkbox" name="portal-user[]" value="<?php esc_attr_e($user['ID']); ?>" class="portal-notify-user-box"><?php echo esc_html($username); ?>
                            </label>
                        </li>
